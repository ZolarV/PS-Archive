...
{ 
 ...
 $folder.items |  
 Select-Object -Property Subject, ReceivedTime, Importance, SenderName, Body, Catagories
} #end function Get-OutlookInbox



#Refrence codes
#$selections = $inbox | select SenderName, Body,  Subject
#$uniqueNames = $inbox | Get-Unique SenderNames
#PS C:\Windows\system32> Foreach($_ in $UniqueNames) {$Threads += @([pscustomObject] @{ Name = $_.Sendername 
#Message = ""} ) }
#foreach($_ in $listss) { if($_[-1].SenderName -eq $threads[-1].Name) { $threads[-1].Message += $_.body} else { $Threads += @([pscustomObject] @{ Name = $_.Sendername 
#Message = $_.body } ) }   } 